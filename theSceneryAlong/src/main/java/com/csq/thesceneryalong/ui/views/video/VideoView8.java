...
import com.csq.thesceneryalong.utils.PxUtil;
 ...
 */
public class VideoView8 extends SurfaceView implements MediaPlayerControl {
    ...
    public VideoView8(Context context) {
        super(context);
        initVideoView();
    }

    public VideoView8(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        initVideoView();
    }

    public VideoView8(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        DebugLog.d("setting size: " + width + 'x' + height);
        ...
        setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					int spx = PxUtil.dip2px(20);
					if(System.currentTimeMillis() - downTime < 150
							&& event.getX() - downX < spx
							&& event.getY() - downY < spx){
						if(isPlaying()){
							pause();
						}else{
							start();
	...
	private void startListenProgress(){
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				...
				if( mCurrentState != STATE_PLAYBACK_COMPLETED && mMediaPlayer != null && mListener != null){
					int cur = getCurrentPosition();
					int tot = getDuration();
					if(tot > 0){
						int pro = cur * 100 / tot;
						mListener.onProgressChanged(pro);
					}
				}
			}
		}, 0, 500);
